...
final class FeedViewControllerTests: XCTestCase {
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view0?.isShowingRetryAction,
                       false, "Expected no retry action for first view once first image loading completes successfully")
        XCTAssertEqual(view1?.isShowingRetryAction, false,
...
private extension FeedViewControllerTests {
    func makeSUT(file: StaticString = #file, line: UInt = #line) -> (sut: FeedViewControllerMVVM,
                                                                     loader: LoaderSpy) {
        let loader = LoaderSpy()
        let sut = FeedUIComposerMVVM.feedComposedWith(feedLoader: loader, imageLoader: loader)
